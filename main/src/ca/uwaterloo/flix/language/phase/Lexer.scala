...
object Lexer {
  ...
  /**
    ...
    */
  def run(root: ReadAst.Root, oldTokens: Map[Ast.Source, Array[Token]], changeSet: ChangeSet, n: Int = 1, fuzzMode: String = "none")(implicit flix: Flix): Validation[Map[Ast.Source, Array[Token]], CompilationMessage] =
    flix.phase("Lexer") {
      ...
      val results = ParOps.parMap(stale.keys)(src => mapN(lex(src))(tokens => {
        val fuzzed = fuzzMode match {
          case "swap" => fuzzSwap(tokens, n)
          case "drop" => fuzzDrop(tokens, n)
          case "prefix" => fuzzPrefix(tokens, n)
          case "postfix" => fuzzPostfix(tokens, n)
          case _ => tokens
        }
        src -> fuzzed
      }))
  ...
  def lex(src: Ast.Source): Validation[Array[Token], CompilationMessage] = {
    ...
    val noComments = s.tokens.filterNot(_.kind.isComment)
    Validation.toSuccessOrSoftFailure(noComments.toArray, errors)
  }
  ...
  /**
    ...
    */
  private def fuzzSwap(tokens: Array[Token], n: Int)(implicit flix: Flix): Array[Token] = {
    ...
    // We fuzz the array by picking two random indices and swapping their tokens n times
    for (_ <- 0 until n) {
      val i = random.nextInt(lastIndex)
      val j = random.nextInt(lastIndex)
      val tmp = copy(i)
      copy(i) = copy(j)
      copy(j) = tmp
    }
  ...
  private def fuzzDrop(tokens: Array[Token], n: Int)(implicit flix: Flix): Array[Token] = {
    // Return immediately if there are few tokens.
    if (tokens.length <= n) {
      return tokens
    }

    val lastIndex = tokens.length - 1 // Note: We don't want to remove the last EOF token.
    val toksToDrop = uniqueRandom(n, 0, lastIndex)
    val newTokens = for ((tok, idx) <- tokens.zipWithIndex if !toksToDrop.contains(idx)) yield tok
    newTokens
  }

  private def fuzzPrefix(tokens: Array[Token], n: Int)(implicit flix: Flix): Array[Token] = {
    // Return immediately if there are few tokens.
    if (tokens.length <= n) {
      return tokens
    }
  ...
  private def fuzzPostfix(tokens: Array[Token], n: Int)(implicit flix: Flix): Array[Token] = {
    ...
    val len = (tokens.length / n).min(tokens.length - 1)
    tokens.drop(len)
  }
}
