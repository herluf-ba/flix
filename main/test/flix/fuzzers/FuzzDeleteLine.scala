...
class FuzzDeleteLine extends AnyFunSuite with TestUtils {
  ...
  coreAndStandardLibrary.foreach {
    case (name, input) => test(s"$name-delete-line")(compileAllLinesLessOne(name, input))
  }
  ...
  private def compileAllLinesLessOne(name: String, stream: java.util.stream.Stream[String]): Unit = {
    ...
    for (i <- 0 until numberOfLines) {
      ...
      flix.addSourceCode(s"$name-drop-line-$i", src)
