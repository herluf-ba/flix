...
class FuzzDuplicateLine extends AnyFunSuite with TestUtils {
...
//    "ToJava.flix" -> Files.lines(Paths.get("./main/src/library/ToJava.flix")),
  ...
  coreAndStandardLibrary.foreach {
    case (name, input) => test(s"$name-duplicate-line")(compileWithDuplicateLine(name, input))
  }
  ...
  private def compileWithDuplicateLine(name: String, stream: java.util.stream.Stream[String]): Unit = {
    ...
    for (i <- 0 until numberOfLines - 1) {
      ...
      try {
        ...
        case e: Throwable =>
          println(s"$name-dublicate-line-$i \n$src")
          throw e
      }
    }
  }
